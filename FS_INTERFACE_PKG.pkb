...
/******************************************************************************
   ...
   1.0        01/19/2018  Vinaykumar Patil 1. Created for Financial Information System of California Interface
                                           ...
                                           2. INSERT_INFAR006_DATA Procedure
                                           3. INSERT_INFAR001_DATA Procedure
             ...
             02/09/2018  Christina Nguyen  1. Added INFAR006 Functions
                                           2. Implemented INSERT_INFAR006_DATA Procedure   
             ...
             02/23/2018  Christina Nguyen  1. Added procedure UPDATE_STATUS_BY_ACCTG_ENTRY_ID
                                           2. Added procedure FS_BATCH_HEADER.
                                           3. Updated Status in Accounting Entry Status properly
            ...
            02/28/2018  Christina Nguyen   1. Changed P_EVENT_TYPE_CODE from GET_INFAR006_DATA
                                           ...
                                           4. Added ACCOUNTING_TRANSACTION.DAILY_POSTING_DATE
                                              ...
                                              for INFAR006 gathering data from last batch process
                                              date.
                                           5. Cleaned up          
            03/15/2018  Vinay Patil        1. INFAR001 has FS_ROW_ID column usage and Constant Values        
            04/02/2018  Vinaykumar Patil   1. Added lookup values based on FISCAL Batch Processing, 
                                              Control the Group items
            04/04/2018  Terence Pan        1. Added check for zero balance to batch driver
            04/05/2018  Terence Pan        1. GET_FS_ALT_ACCT now will generate the correct alt acct 
                                              for A/R Contingent and A/R Others
           05/01/2018   Vinay Patil        1. Added Log message to CARS ERROR LOG table in CARS schem in FS_BATCH_DRIVE procedure
           05/09/2018   Raman Nakarmi      1. Added a if else statement for GET_INFAR001_DATA to account for reversals.
           05/10/2018   Christina Nguyen   1. Added missing AR Others Payment, AR Others Payment Rejection,
                                              Payment Adjustment, and AR Others Contingent in INFAR006.
                                           2. Included Entry Type Code WO, CRYEC, and DRYEC into INFAR006
                                              Amount, A, and U columns.
                                           3. Added logic to get Original Invoice Date as ASOF_DT for
                                              sub system PVERA and SIMS.
           05/11/2018   Christina Nguyen   1. Removed Event 900 in FS_PROCESS_INFAR006_SETUP and
                                              FS_PROCESS_INFAR006_ADJUST 
                                           2. Included Rejection Events for getting Amount, U, and V value
                                           3. Removed Event 020 in populate CustomerId and ASOF_DT in 
                                              GET_INFAR006_DATA procedure.
                                           4. Added IS_REVERSAL_EVENT function to check for both 900 and
                                              Rejection event types for INFAR006.
                                           5. Fixed NULL for CALOSHA program unit in GET_FS_LINE_NUMBER
           05/11/2018   Vinay Patil        1. Added function FUNC_GET_FISCAL_MONTH, FUNC_GET_FISCAL_YEAR and FUNC_GET_PRIOR_FISCAL_YEAR
                                           2. Added Error logging into the functions
                                           3  Added return parameter for success flag and message for FS_BATCH_HEADER procedure..
                                           4. Capture the Rollback in the Batch Creation and accordingly check in the FS_BATCH_DRIVER to run related batches only.
           05/12/2018    Raman Nakarmi     1. Removed event 130 from adjustment, it was mapped incorrectly, and was only supposed to be in setups
           05/14/2018   Christina Nguyen   1. Cleaned up GET_FS_LINE_NUMBER for CALOSHA program unit.
                                           2. Posted detail error for missing CUSTOMER_ID in GET_FS_CUSTOMER_ID function
                                           3. Post detail error for missing FS_ADDRESS_SEQUENCE_NUMBER in GET_FS_ADDRESS_ID function
                                           4. Skipped process INSERT_INFAR006_DATA procedure in GET_INFAR006_DATA procedure if
                                              CUSTOMER_ID or FS_ADDRESS_SEQUENCE_NUMBER does not have value.
           05/16/2018  Christina Nguyen    1. Added UPD_EVENT_900_STATUS to set Status NEW to Status NOT_TRANSMITTED
                                           2. Replaced p_errorMessage  => null with p_errorMessage  => SQLERRM for more error detail.
           05/18/2018  Terence Pan         1. Added fix for GET_FS_GL_REV_SRC to handle reclass and write off
                                           2. Added fix for PP_SW = 'Y' Yes for Billed payments and 'N' for Direct Journal payments
		   05/22/2018  Raman Nakarmi       1. Commented out driver procedure for EVENT 135 and 160 per Lis's request.
           05/23/2018  Vinay Patil         1. Modified logic for Error Handling and OUT parameter value setting.
           05/24/2018  Vinay Patil         1. Modified logic for INFAR006 Cursor, removed the cursor parameter and modified the filter.
                                           2. Consolidated the logic to simpler queries and reusable methods
                                           3. Removed comments and variable assignments not required.
                                           4. For INFAR001, removed the record count cursor, Accounting Entry Id for 001 row.
                                           5. Made the code readable by alignment and usage of IN clause where needed.
           05/25/2018  Vinay Patil         1. Modified logic for INFAR001 Cursor, fetch Piscal Year based on Payment Transaction Code
           05/29/2018  Terence Pan         1. Modified logic for INFAR001 Cursor to not return Customer Id for UNBILLED
           05/30/2018  Terence Pan         1. Signs were wrong for DR AROT coming in from AR Others Payment Reversal events
            06/01/2018  Vinay Patil        1. Modified logic for INSERT_INFAR001_DATA procedure for line 2,3,4 and 5 to accept and populate AR_ROOT_DOCUMENT
           06/06/2018  Terence Pan         1. FUNC_GET_FS_DEPOSIT returns 'R' for EDF, LOCKBOX and credit cards
           06/07/2018  Terence Pan         1. Missing mandatory information at '001' level for INFAR001 will not generate 
                                                records and be logged instead
		   07/06/2018    Raman Nakarmi     1. Moved functions for zero balance, and 900 events after update statement in batch_Driver procedure									
*****************************************************************************/
...
-- INFAR001 FUNCTIONS
    ...
    ******************************************************************************/        
        ...
        */
        ...
        CASE 
            WHEN receiptType IN ('EFT','VISA_MASTER_CREDIT_CARD', 'AMEX_CREDIT_CARD', 
                                ...
                                'EDF', 'ACCOUNT_TRANSFER')  THEN  
                                                                                fsDepositType := 'R';
            --WHEN receiptType IN () THEN                       fsDepositType := 'T';
    ...
    FUNCTION GET_FS_CUSTOMER_ID(varReferenceDoc VARCHAR2) RETURN VARCHAR2 IS
    
        varCustId   PARTICIPANT_ROLE.CUST_ID%TYPE;
        varMsg      VARCHAR2(500);
    ...
    BEGIN
        ...
        SELECT  PARTICIPANT_ROLE.CUST_ID 
        INTO    varCustId
        FROM    EVENT
            INNER JOIN PARTICIPANT_ROLE ON PARTICIPANT_ROLE.EVENT_ID = EVENT.EVENT_ID
            INNER JOIN EVENT_TYPE       ON EVENT.EVENT_TYPE_ID       = EVENT_TYPE.EVENT_TYPE_ID
        ...
        -- 5/24/2018, Vinay Patil: Get the Address Sequence Number based on AR setup event.            
        IF (varCustId IS NULL) THEN
        
            varMsg := 'Invoice Number: ' || varReferenceDoc || ' does not have Fiscal Customer ID on the AR Setup event transaction.';
            
            LOG_CARS_ERROR(
                    p_errorLevel    => '3',
                    p_severity      => c_HIGH_SEVERITY,
                    p_errorDetail   => 'Missing Customer Id Information',
                    p_errorCode     => '5000',
                    p_errorMessage  => varMsg,
                    p_dataSource    => c_CARS_DB
	                );  
                    
           DBMS_OUTPUT.PUT_LINE('GET_FS_CUSTOMER_ID function: '|| varMsg);         
        END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Consider logging the error and then re-raise
            -- Removed RAISE, added return null.
            DBMS_OUTPUT.PUT_LINE('GET_FS_CUSTOMER_ID : '||SQLERRM);
            
           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL GET_FS_CUSTOMER_ID function could not return Fiscal Customer ID on the AR Setup event transaction.',
                p_errorCode     => '5000',
                p_errorMessage  => 'GET_FS_CUSTOMER_ID : '||SQLERRM,
                p_dataSource    => c_CARS_DB
              );
    
            RETURN NULL;
            
    END GET_FS_CUSTOMER_ID;   
    
    FUNCTION GET_FS_AS_OF_DATE(varReferenceDoc VARCHAR2) RETURN DATE IS     
    
        varEventDate EVENT.EVENT_DATE%TYPE;
    
    BEGIN

        -- 5/24/2018, Vinay Patil: Get the Event Date of AR setup event.
        ...
        RETURN TRUNC(varEventDate);
        
    EXCEPTION
        WHEN OTHERS THEN
            -- Consider logging the error and then re-raise
            -- Removed RAISE, added return null.
            DBMS_OUTPUT.PUT_LINE('GET_FS_AS_OF_DATE : '||SQLERRM);
            
           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL GET_FS_AS_OF_DATE function could not return Event date',
                p_errorCode     => '5000',
                p_errorMessage  => 'GET_FS_AS_OF_DATE : '||SQLERRM,
                p_dataSource    => c_CARS_DB
              );
    
            RETURN NULL;
            
    END GET_FS_AS_OF_DATE;   
    
    FUNCTION GET_FS_DIR_PAYMENT_TERMS(varRefValue VARCHAR2) RETURN VARCHAR2 IS
    
        varPaymentTerm REFERENCE_CODE_LIST.REFERENCE_CODE%TYPE;
    
    BEGIN

        SELECT  RCL.REFERENCE_CODE 
        INTO    varPaymentTerm 
        FROM    REFERENCE_CODE_LIST RCL
        WHERE   RCL.REFERENCE_VALUE = varRefValue 
        AND     RCL.DOMAIN_NAME     = c_DIR_DUE_DATE;
        
        RETURN varPaymentTerm;

    EXCEPTION
        WHEN OTHERS THEN
            -- Consider logging the error and then re-raise
            -- Removed RAISE, added return null.
            DBMS_OUTPUT.PUT_LINE('GET_FS_DIR_PAYMENT_TERMS : '||SQLERRM);
            
           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL GET_FS_DIR_PAYMENT_TERMS function could not return payment term',
                p_errorCode     => '5000',
                p_errorMessage  => 'GET_FS_DIR_PAYMENT_TERMS : '||SQLERRM,
                p_dataSource    => c_CARS_DB
              );
    
            RETURN NULL;

    END GET_FS_DIR_PAYMENT_TERMS;
    ...
    FUNCTION GET_INLIST(varInput VARCHAR2) RETURN VARCHAR2 IS

        varInList       VARCHAR2(100);
        varValue        VARCHAR2(3);
        varQuote        VARCHAR2(1);
        varDelimeter    VARCHAR2(1);
        
        -- Parse the list of value based on comma seperated list
        CURSOR eventType_Cursor IS
            SELECT TRIM(regexp_substr (varInput, '[^,]+',1, ROWNUM)) str 
            FROM DUAL 
            CONNECT BY LEVEL <= regexp_count (varInput, '[^,]+');
    BEGIN
        varQuote        := chr(39);
        varDelimeter    := ',';
        varInList       := '';
        
        --Open eventType_Cursor
        OPEN eventType_Cursor;
        LOOP
            FETCH eventType_Cursor INTO varValue;
            EXIT WHEN eventType_Cursor%NOTFOUND;
            
            IF(LENGTH(varInList) > 0) THEN
                varInList := varInList || varDelimeter;
            END IF;

            varInList := varInList || varQuote || varValue ||varQuote;

        END LOOP;
      
        DBMS_OUTPUT.PUT_LINE('In List value: ' || varInList);
        RETURN varInList;
    ...
    EXCEPTION
        WHEN OTHERS THEN
           ...
           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL GET_INLIST function could not return in list',
                p_errorCode     => '5000',
                p_errorMessage  => 'FUNC_GET_FS_PAYMENT_METHOD : '||SQLERRM,
                p_dataSource    => c_CARS_DB
              );
    ...
    -- Check whether the given Event Type Code is Reversal Event 900 or Rejection Events
    FUNCTION IS_REVERSAL_EVENT(varEventTypeCode VARCHAR2) RETURN NUMBER AS
        reversalEvent NUMBER := 0;
    BEGIN
        IF (varEventTypeCode = c_EVENT_900 OR 
            varEventTypeCode = c_EVENT_325 OR 
            varEventTypeCode = c_EVENT_330 OR 
            varEventTypeCode = c_EVENT_331) THEN
    ...
    ******************************************************************************/
    ...
    FUNCTION FUNC_GET_FISCAL_YEAR(varRootDoc IN VARCHAR2) RETURN VARCHAR2 IS
    /******************************************************************************
       PURPOSE:    Convert the given root document event date to fiscal year name.
                   This function is called inside another function
                   FUNC_GET_FISCAL_PERIOD_ID so please do not remove this function.

       Ver        Date        Author           Description
       ---------  ----------  ---------------  ------------------------------------
       1.0        8/29/2013   Christina Nguyen 1. Created this function.
       1.1        11/27/2013  Mike Jovero      1.Revised Select into varEventDate...
       1.3        6/30/2016   Mike Jovero      1. Now forces to use Root Document regardless of input. 
       1.4        8/31/2016   Mike Jovero      1. Optimized queries
    ******************************************************************************/
        varEventDate    DATE;
        varRefDoc       VARCHAR2(100);
        varFiscalYear   VARCHAR2(4);
        tempMonth       VARCHAR2(2);
        tempYear        VARCHAR2(4);
        tempNumberMonth NUMBER;
    ...
    BEGIN
        ...
        IF (varRootDoc IS NOT NULL) THEN
            ...
            -- get the root document in case of Event 515's
            --SELECT AR_ROOT_DOCUMENT into varRefDoc FROM EVENT WHERE CURRENT_DOCUMENT = varRootDoc;

            --SELECT  /*+INDEX (EVENT EVENT_REFERENCE_DOC_IX EVENT_CURRENT_DOC_IX)*/ E.EVENT_DATE into varEventDate
            --FROM (select * from event where CURRENT_DOCUMENT = varRefDoc ) E WHERE Level = 1 
            --CONNECT BY NOCYCLE PRIOR E.REFERENCE_DOCUMENT = E.CURRENT_DOCUMENT  ;
        ...
        ELSE
            varFiscalYear   := NULL;
            varMessage      := 'FUNC_GET_FISCAL_YEAR: Invalid input parameter. Root Document '||varRootDoc||' must have a value';
            DBMS_OUTPUT.PUT_LINE(varMessage);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Consider logging the error and then re-raise
            varFiscalYear := NULL;
            varMessage    := 'FUNC_GET_FISCAL_YEAR: '||' varRootDoc '||varRootDoc||' '||SQLERRM;
            DBMS_OUTPUT.PUT_LINE(varMessage);

           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL FUNC_GET_FISCAL_YEAR function could not return financial year',
                p_errorCode     => '5000',
                p_errorMessage  => varMessage,
                p_dataSource    => c_CARS_DB
              );
    ...
    END FUNC_GET_FISCAL_YEAR;

    FUNCTION FUNC_GET_FISCAL_MONTH(varDate IN DATE) RETURN VARCHAR2 IS
    /******************************************************************************
       PURPOSE:    Convert the given date to fiscal month name.

       Ver        Date        Author           Description
       ---------  ----------  ---------------  ------------------------------------
       1.0        8/29/2013   Christina Nguyen  1. Created this function.
    ******************************************************************************/
        varFiscalMonth  VARCHAR2(2);
        tempMonth       VARCHAR2(2);
        tempNumberMonth NUMBER;
        varMessage      VARCHAR2(200);
    ...
    BEGIN
        ...
        IF (varDate IS NOT NULL) THEN
            ...
            varFiscalMonth  := null;
            tempMonth       := to_char(varDate, 'MM');
            tempNumberMonth := to_number(tempMonth);

            if(tempNumberMonth > 6) then
                varFiscalMonth := lpad(tempNumberMonth - 6, 2, '0');
            else
                varFiscalMonth := lpad(tempNumberMonth + 6, 2, '0');
            end if;
        ...
        ELSE
            varFiscalMonth  := NULL;
            varMessage      := 'FUNC_GET_FISCAL_MONTH: Invalid input parameter. Input Date '||varDate||' must have a value';
            DBMS_OUTPUT.PUT_LINE(varMessage);

        END IF;

        RETURN varFiscalMonth;

    EXCEPTION
        WHEN OTHERS THEN
            -- Consider logging the error and then re-raise
            varFiscalMonth := NULL;
            varMessage      := 'FUNC_GET_FISCAL_MONTH: '||' varDate '||varDate||' '||SQLERRM;
            DBMS_OUTPUT.PUT_LINE(varMessage);

           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL FUNC_GET_FISCAL_MONTH function could not return financial month',
                p_errorCode     => '5000',
                p_errorMessage  => varMessage,
                p_dataSource    => c_CARS_DB
              );
    ...
    END FUNC_GET_FISCAL_MONTH;

    --5/24/2018, Vinay Patil:  Procedure to return Customer Id, Address Sequence Number, AS OF Date and FISCAL Year
    --                      : Input parameter, the Invoice or AR Root Document, Current Event Date from main cursor and Event Type Code and the Program Unit Code
    PROCEDURE FS_AR_SETUP_DATA(
            P_AR_ROOT_DOCUMENT      EVENT.AR_ROOT_DOCUMENT%TYPE,
            P_CUR_EVENT_DATE        EVENT.EVENT_DATE%TYPE, 
            P_EVENT_TYPE_CODE       EVENT_TYPE.EVENT_TYPE_CODE%TYPE,
            P_PROGRAM_UNIT          EVENT_TYPE.PROGRAM_UNIT_CODE%TYPE, 
            P_CUST_ID           OUT PARTICIPANT_ROLE.CUST_ID%TYPE,
            P_ADDRESS_SEQ_NUM   OUT ADDRESS.FS_SEQUENCE_NUMBER%TYPE,
            P_AS_OF_DATE        OUT EVENT.EVENT_DATE%TYPE,
            P_FISCAL_YEAR       OUT FISCAL_PERIOD.FISCAL_YEAR_NAME%TYPE,
            P_PRIOR_FISCAL_YEAR OUT FISCAL_PERIOD.FISCAL_YEAR_NAME%TYPE,
            P_SUCCESS_FLAG      OUT VARCHAR2,
            P_MESSAGE           OUT VARCHAR2
            ) AS

            V_CUST_ID           PARTICIPANT_ROLE.CUST_ID%TYPE;
            V_ADDRESS_SEQ_NUM   ADDRESS.FS_SEQUENCE_NUMBER%TYPE;
            V_AS_OF_DATE        EVENT.EVENT_DATE%TYPE;
            V_FISCAL_YEAR       FISCAL_PERIOD.FISCAL_YEAR_NAME%TYPE;
            V_PRIOR_FISCAL_YEAR FISCAL_PERIOD.FISCAL_YEAR_NAME%TYPE;
            V_CURR_FISCAL_YEAR  FISCAL_PERIOD.FISCAL_YEAR_NAME%TYPE;
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('P_PROGRAM_UNIT '||P_PROGRAM_UNIT||' P_AR_ROOT_DOCUMENT '||P_AR_ROOT_DOCUMENT||' P_EVENT_TYPE_CODE '||P_EVENT_TYPE_CODE||' P_CUR_EVENT_DATE '||P_CUR_EVENT_DATE);
        
        -- To avoid repeated call to fetch the AR Setup Event to find Customer Id, Address Id and As Of Date.
        SELECT  PARTICIPANT_ROLE.CUST_ID,
                ADDRESS.FS_SEQUENCE_NUMBER,
               -- All non-AR Setup, Non-CALOSHA AR events
                CASE  
                    WHEN (P_EVENT_TYPE_CODE NOT IN ( c_EVENT_010,c_EVENT_015) AND 
                         (P_PROGRAM_UNIT != c_PU_CALOSHA)) THEN                          EVENT.EVENT_DATE
                    WHEN (P_EVENT_TYPE_CODE IN ( c_EVENT_010,c_EVENT_015)) THEN          EVENT.EVENT_DATE
                ELSE                                                                     P_CUR_EVENT_DATE   
                END AS FS_AS_OF_DATE,
                
                -- Find the Financial Year based on the AR Setup date
                CASE  WHEN (EXTRACT(MONTH from EVENT.EVENT_DATE) between 1 and 6) THEN   TO_NUMBER(EXTRACT(YEAR from EVENT.EVENT_DATE))-1
                ELSE                                                                     TO_NUMBER(EXTRACT(YEAR from EVENT.EVENT_DATE))
                END FS_FISCAL_YEAR,
               
                -- Find the Financial Year based on the AR Setup date
                CASE  WHEN (EXTRACT(MONTH from V_SYSDATE) between 1 and 6) THEN          TO_NUMBER(EXTRACT(YEAR from V_SYSDATE))-1
                ELSE                                                                     TO_NUMBER(EXTRACT(YEAR from V_SYSDATE))
                END FS_CUR_FISCAL_YEAR
        ...
        FROM    EVENT
            ...
            INNER JOIN EVENT_TYPE       ON EVENT.EVENT_TYPE_ID         = EVENT_TYPE.EVENT_TYPE_ID
        WHERE EVENT_TYPE.EVENT_TYPE_CODE IN (c_EVENT_010, c_EVENT_015)
          AND CURRENT_DOCUMENT          = P_AR_ROOT_DOCUMENT
          AND AR_ROOT_DOCUMENT          = P_AR_ROOT_DOCUMENT;
        ...
        -- 5/25/2018, Vinay Patil: Added calculation of prior year.
        -- Compare the Setup AR event or the Invoice Date with Current Date. If Invoice date is equal or older than 2 years then it's prior year.        
        IF ((V_CURR_FISCAL_YEAR - V_FISCAL_YEAR) >= 2) THEN
            V_PRIOR_FISCAL_YEAR := V_FISCAL_YEAR;
        ELSE
            V_PRIOR_FISCAL_YEAR := V_CURR_FISCAL_YEAR;        
        END IF;
       P_PRIOR_FISCAL_YEAR :=  V_PRIOR_FISCAL_YEAR;

        V_SUCCESS_FLAG  := c_YES;
        V_MESSAGE       := 'P_PROGRAM_UNIT '||P_PROGRAM_UNIT||' P_AR_ROOT_DOCUMENT '||P_AR_ROOT_DOCUMENT||' P_EVENT_TYPE_CODE '||P_EVENT_TYPE_CODE||' P_CUR_EVENT_DATE '||P_CUR_EVENT_DATE||
                            ' V_ADDRESS_SEQ_NUM '||V_ADDRESS_SEQ_NUM||
                            ' V_AS_OF_DATE '||V_AS_OF_DATE||' V_CUST_ID '||V_CUST_ID||' V_FISCAL_YEAR '||V_FISCAL_YEAR||' V_PRIOR_FISCAL_YEAR '||V_PRIOR_FISCAL_YEAR;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            DBMS_OUTPUT.PUT_LINE(V_MESSAGE||' '||SQLERRM);
           ...
           FS_INTERFACE_PKG.LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL FS_AR_SETUP_DATA procedure could not return Fiscal Address Sequence Number, Customer Id and AS OF Date, Fiscal Year on the AR Setup event transaction',
                p_errorCode     => '5000',
                p_errorMessage  => V_MESSAGE||' '||SQLERRM,
                p_dataSource    => c_CARS_DB
              );

    END FS_AR_SETUP_DATA;  
    ...
    PROCEDURE UPDATE_ACCTG_ENTRY_STATUS(
        ...
        varInList           VARCHAR2(100);
        varSql              VARCHAR2(2500);

        varQuote   CONSTANT CHAR := chr(39);
        varLF      CONSTANT CHAR := chr(10);
        
    BEGIN
        -- put single quote around each event type code
        varInList := GET_INLIST(varEventType);
        DBMS_OUTPUT.PUT_LINE('varInList: ' || varInList);
    
        varSql := 'UPDATE ACCOUNTING_ENTRY_STATUS '||
                  ' SET MODIFIED_BY = USER, MODIFIED_DATE = sysdate, FS_PROCESS_STATUS = ' || varQuote || varStatus || varQuote || ', FS_PROCESS_DATE = ';
        
        IF (varBatchDate IS NULL ) THEN
            varSql :=  varSql || 'NULL' || varLF;
        ELSE
            varSql :=  varSql || varQuote ||  varBatchDate || varQuote;
        END IF; 
        ...
        varSql :=  varSql || '
         ...
         LEFT JOIN PARTICIPANT_ROLE 
            ON (EVENT.EVENT_ID                              = PARTICIPANT_ROLE.EVENT_ID)   
         LEFT JOIN NOTE ON NOTE.EVENT_ID                    = EVENT.EVENT_ID                  
         WHERE ACCOUNTING_ENTRY_TYPE.ACCTG_TRANSACT_CODE = '    || varTransactionCode               ||
            ' AND EVENT_TYPE.EVENT_TYPE_CODE IN ('              || varInList    || ') '             ||
            ' AND EVENT_TYPE.PROGRAM_UNIT_CODE = '              || varQuote     || varProgramUnit   || varQuote || 
            ' AND ACCOUNTING_ENTRY_STATUS.FS_PROCESS_STATUS = ' || varQuote     || varCurrentStatus || varQuote || 
            ' AND ( PARTICIPANT_ROLE.PARTICIPANT_ROLE_TYPE_CODE IN (' 
                                                                || varQuote     || 
                                                    'RES_PARTY' || varQuote     || 
                                                    ', '        || varQuote     || 
                                                    'PAYOR'     || varQuote     ||
                                                    ') '        ||  
            ' OR PARTICIPANT_ROLE.PARTICIPANT_ROLE_TYPE_CODE IS NULL ) ) ' ;
            
            --'AND ((NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE ' || 
            --varQuote || '%Reclassified%' || varQuote || ') ' ||  varLF ||
            -- 5/24/2018, Vinay Patil: This is note NOT required as this is being handled in the Accounting Entry Status tables.
            --'AND (NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE ' || varQuote || '%Refunded with Warrant Number%' || varQuote || ') ' || 
            --'AND (NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE ' || varQuote || '%not transmitted to CALSTARS%'  || varQuote || ') )';
    
       DBMS_OUTPUT.PUT_LINE('UPDATE_ACCTG_ENTRY_STATUS: Dynamic sql updated Accounting Entry Status with status : ' 
                                || varStatus
                                ||' varBatchDate '      || varBatchDate
                                ||' varTransactionCode '|| varTransactionCode 
                                ||' varProgramUnit '    || varProgramUnit 
                                ||' varCurrentStatus '  || varCurrentStatus
                                );
    
       EXECUTE IMMEDIATE varSql;

       DBMS_OUTPUT.PUT_LINE('UPDATE_ACCTG_ENTRY_STATUS: Dynamic sql updated rows ' || sql%rowcount);
    
    EXCEPTION
        WHEN OTHERS THEN
            -- Consider logging the error and then re-raise            
            varErrorMsg := 
                'UPDATE_ACCTG_ENTRY_STATUS: Could not UPDATE ACCOUNTING_ENTRY_STATUS for ' || varStatus ||
                ' STATUS on Batch Date ' || varBatchDate || ', Tranasction Code ' || varTransactionCode ||
                ', and Event Type Code ' || varEventType||' '|| SQLERRM;
                
            DBMS_OUTPUT.PUT_LINE('UPDATE_ACCTG_ENTRY_STATUS Error: ' || varErrorMsg);
            
           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL UPDATE_ACCTG_ENTRY_STATUS procedure could not update status',
                p_errorCode     => '5000',
                p_errorMessage  => varErrorMsg,
                p_dataSource    => c_CARS_DB
              );
    
    END UPDATE_ACCTG_ENTRY_STATUS;  
    ...
    FUNCTION FS_BATCH_TRIAL_RUN RETURN NUMBER IS
        varCount NUMBER;
    BEGIN
    
        -- The Query gives the count of records eligible for FISCAL data processing.
        SELECT  COUNT(*) 
        INTO    varCount
        ...
        WHERE   ACCTG_ENTRY_ID IN (
               ...
               FROM     ACCOUNTING_ENTRY
                   ...
                   LEFT JOIN PARTICIPANT_ROLE
                        ON (EVENT.EVENT_ID = PARTICIPANT_ROLE.EVENT_ID)  
                   LEFT JOIN NOTE 
                        ON NOTE.EVENT_ID = EVENT.EVENT_ID                    
               WHERE ACCOUNTING_ENTRY_STATUS.FS_PROCESS_STATUS = c_STATUS_NEW
               AND   ACCOUNTING_ENTRY_STATUS.FS_PROCESS_DATE IS NULL
               AND  (PARTICIPANT_ROLE.PARTICIPANT_ROLE_TYPE_CODE IN ('RES_PARTY', 'PAYOR') OR PARTICIPANT_ROLE.PARTICIPANT_ROLE_TYPE_CODE IS NULL)
                );
           --AND ((NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE '%Reclassified%') 
           -- 5/24/2018, Vinay Patil: This is note NOT required as this is being handled in the Accounting Entry Status tables.
           --AND  (NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE '%Refunded with Warrant Number%')
           --AND  (NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE '%not transmitted to CALSTARS%'))

        DBMS_OUTPUT.PUT_LINE(' FS_BATCH_TRIAL_RUN: Total rows = '||varCount);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            DBMS_OUTPUT.PUT_LINE('FS_BATCH_TRIAL_RUN : Error: '||SQLERRM);
            
           LOG_CARS_ERROR(
                p_errorLevel    => '3',
                p_severity      => c_HIGH_SEVERITY,
                p_errorDetail   => 'FISCAL FS_BATCH_TRIAL_RUN function could not return count ',
                p_errorCode     => '5000',
                p_errorMessage  => 'FS_BATCH_TRIAL_RUN : Error: '||SQLERRM,
                p_dataSource    => c_CARS_DB
              );
        
    END FS_BATCH_TRIAL_RUN;
    ...
    BEGIN
       INSERT INTO BATCH (
        ...
        varBatchNumber, 
        '00000', 
        FUNC_GET_FISCAL_MONTH(V_SYSDATE), 
        c_BATCH_COMPLETE, 
        c_CARS_DB,
        c_USER, 
        V_SYSDATE
        ...
        V_SUCCESS_FLAG := C_YES;
        V_MESSAGE      := 'FS_BATCH_HEADER: Batch created for '|| varBatchType||' with batch id '||varBatchId;

        P_SUCCESS_FLAG     := V_SUCCESS_FLAG;
        P_MESSAGE          := V_MESSAGE;
        
        DBMS_OUTPUT.PUT_LINE('FS_BATCH_HEADER : V_SUCCESS_FLAG = '||V_SUCCESS_FLAG||' '||V_MESSAGE); 
                
    EXCEPTION
           WHEN OTHERS THEN
                V_MESSAGE := 'FS_BATCH_HEADER: Could not INSERT data into BATCH table for Batch Id ' || varBatchId ||
                    ', Batch Number ' || varBatchNumber || ', and Batch Type Code ' || varBatchType||' '||SQLERRM;
                ...
                -- 5/8/2018, Raman Nakarmi: Log the message to CARS ERROR LOG table in CARS schema.
                LOG_CARS_ERROR(
                    p_errorLevel    => '3',
                    p_severity      => c_HIGH_SEVERITY,
                    p_errorDetail   => 'FISCAL FS_BATCH_HEADER procedure did not create the batch record',
                    p_errorCode     => '5000',
    ...
    PROCEDURE FS_BATCH_DRIVER(
        ...
        varINFAR001BatchId          NUMBER;
        varINFAR006sBatchId         NUMBER;
        varINFAR006aBatchId         NUMBER;
        varBatchNumber              INTEGER;
        varBatchTransactSeqNumber   INTEGER;
        varBatchIdDateDiff          NUMBER;
        varBatchDate                DATE;
        varBatchNumberStr           VARCHAR2(3);
        varBatchDateStr             BATCH.BATCH_DATE%TYPE;
        varProgramUnit              EVENT_TYPE.PROGRAM_UNIT_CODE%TYPE;
        varLastBatchDate            DATE;

        -- 5/1/2018, Vinay Patil: Added parameter for error logging in CARS system.Varaible set for successful transmission
        V_SUCCESS_FLAG              VARCHAR2(1);
        V_MESSAGE                   VARCHAR2(500);
        V_LEVEL                     CARS_ERROR_LOG.ERROR_LEVEL%TYPE     := '1';    
        V_SEVERITY                  CARS_ERROR_LOG.SEVERITY%TYPE        := c_LOW_SEVERITY;
        V_ERROR_CODE                CARS_ERROR_LOG.ERROR_CODE%TYPE      := '0000';
        V_ERROR_DETAIL              CARS_ERROR_LOG.ERROR_DETAIL%TYPE    := 'FISCAL Interface data processing was successful';
        
       -- defined Program Unit Code Cursor

       CURSOR programUnit_Cursor IS
           SELECT DISTINCT EVENT_TYPE.PROGRAM_UNIT_CODE
             FROM EVENT_TYPE
             INNER JOIN EVENT ON EVENT.EVENT_TYPE_ID = EVENT_TYPE.EVENT_TYPE_ID
             INNER JOIN ACCTG_TRANSACT_EVENT_ASSOC ON EVENT.EVENT_ID = ACCTG_TRANSACT_EVENT_ASSOC.EVENT_ID
             INNER JOIN ACCOUNTING_ENTRY ON ACCOUNTING_ENTRY.ACCTG_TRANSACTION_ID = ACCTG_TRANSACT_EVENT_ASSOC.ACCTG_TRANSACTION_ID
             INNER JOIN ACCOUNTING_ENTRY_STATUS
               ON (ACCOUNTING_ENTRY_STATUS.ACCTG_ENTRY_ID =
                       ACCOUNTING_ENTRY.ACCTG_ENTRY_ID) 
             LEFT JOIN NOTE ON NOTE.EVENT_ID = EVENT.EVENT_ID
             WHERE ACCOUNTING_ENTRY_STATUS.FS_PROCESS_STATUS = c_STATUS_NEW
             AND ACCOUNTING_ENTRY_STATUS.FS_PROCESS_DATE IS NULL
             ;
             --AND ACCOUNTING_ENTRY.POSTING_DATE > lastBatchDate
             --AND ((NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE '%Reclassified%') 
             -- 5/24/2018, Vinay Patil: This is note NOT required as this is being handled in the Accounting Entry Status tables.
             --AND (NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE '%Refunded with Warrant Number%')
             --AND (NOTE.NOTE_TEXT IS NULL OR NOTE.NOTE_TEXT NOT LIKE '%not transmitted to CALSTARS%');
    ...
    BEGIN
       DBMS_OUTPUT.PUT_LINE('Begin FS_BATCH_DRIVER');
       V_SUCCESS_FLAG := c_NO;
       
       -- test if there are any transactions_entries to be batch for the given date,
       -- if none exit this procedure.
       IF (FS_BATCH_TRIAL_RUN < 0 ) then
       ...
       -- VINAY PATIL : 5/23/2018: Combined all the data and sequence generation query.
       -- Both INFAR001 and INFAR006 share Batch Number
       -- get BATCH_ID and create BATCH header for INFAR001
       SELECT   SYSDATE,
                BATCH_NUMBER_SEQ.NEXTVAL , 
                BATCH_ID_SEQ.NEXTVAL
       INTO     varBatchDate,
                varBatchNumber , 
                varINFAR001BatchId
       FROM     DUAL;
       ...
       --Set zero balance/NEW to NOT_TRANSMITTED
       --zero balances will be failed in FISCAL system.
       UPD_INFAR006_ZERO_BAL(varBatchDate, V_SUCCESS_FLAG, V_MESSAGE);

       -- Update Event 900 status to NOT_TRANSMITTED per ACCOUNTING's request
       UPD_EVENT_900_STATUS (varBatchDate, V_SUCCESS_FLAG, V_MESSAGE);   
       
       -- Update Event 135, 160 status to NOT_TRANSMITTED per ACCOUNTING's request  
       UPD_EVENT_WO_STATUS (varBatchDate, V_SUCCESS_FLAG, V_MESSAGE);

       varBatchDateStr := TO_CHAR(varBatchDate, 'YYYYMMDD');
       
       varLastBatchDate:= TRUNC(varBatchDate);
       
       varBatchNumberStr :=  LPAD(TO_CHAR(varBatchNumber), 3, 0);
       
       DBMS_OUTPUT.PUT_LINE('varBatchDate: ' || varBatchDate||' varLastBatchDate: ' || varLastBatchDate||' varBatchDateStr: ' || varBatchDateStr
                               ||' varBatchNumberStr: ' || varBatchNumberStr||' varINFAR001BatchId: ' || varINFAR001BatchId);
       
       FS_BATCH_HEADER(varINFAR001BatchId, varBatchNumberStr, varBatchDateStr, C_INFAR018_CARS,V_SUCCESS_FLAG,V_MESSAGE );

       IF (V_SUCCESS_FLAG = C_YES) THEN  
           --Insert '000' header record for INFAR001
           INSERT_INFAR001_DATA (varINFAR001BatchId, varBatchDateStr, varBatchDate, c_FS_DIR_BIZ_UNIT, 0, 0, V_SUCCESS_FLAG, V_MESSAGE);
           
           DBMS_OUTPUT.PUT_LINE('Created Batch Header for INFAR001 '||varINFAR001BatchId);
           
       ELSE
           DBMS_OUTPUT.PUT_LINE('Creating Batch Record '||varINFAR001BatchId||' for INFAR001 failed '||V_MESSAGE);
            varINFAR001BatchId := NULL;    
       END IF;
       
       -- get BATCH_ID and create BATCH header for INFAR006 Setup
       SELECT BATCH_ID_SEQ.NEXTVAL INTO varINFAR006sBatchId FROM DUAL;
       DBMS_OUTPUT.PUT_LINE('varINFAR006sBatchId: ' || varINFAR006sBatchId);
       
       FS_BATCH_HEADER(varINFAR006sBatchId, varBatchNumberStr, varBatchDateStr, c_INFAR006_CARS_S,V_SUCCESS_FLAG,V_MESSAGE );

       IF (V_SUCCESS_FLAG = C_YES) THEN  
           DBMS_OUTPUT.PUT_LINE('Created Batch Header for INFAR006 for setup '||varINFAR006sBatchId);           
           
       ELSE
           DBMS_OUTPUT.PUT_LINE('Creating Batch Record for INFAR006 for setup '||varINFAR006sBatchId||' for INFAR001 failed '||V_MESSAGE);
            varINFAR006sBatchId := NULL;    
       END IF;
       
       -- get BATCH_ID and create BATCH header for INFAR006 Adjustment
       SELECT BATCH_ID_SEQ.NEXTVAL INTO varINFAR006aBatchId FROM DUAL;
       DBMS_OUTPUT.PUT_LINE('varINFAR006aBatchId: ' || varINFAR006aBatchId);
        
       FS_BATCH_HEADER(varINFAR006aBatchId, varBatchNumberStr, varBatchDateStr, c_INFAR006_CARS_A,V_SUCCESS_FLAG,V_MESSAGE );

       IF (V_SUCCESS_FLAG = C_YES) THEN  
           DBMS_OUTPUT.PUT_LINE('Created Batch Header for INFAR006 for adjustment '||varINFAR006aBatchId);           
           
       ELSE
           DBMS_OUTPUT.PUT_LINE('Creating Batch Record for INFAR006 for adjustment '||varINFAR006aBatchId||' for INFAR001 failed '||V_MESSAGE);
            varINFAR006aBatchId := NULL;    
       END IF;


       IF (varINFAR001BatchId IS NOT NULL) OR (varINFAR006sBatchId IS NOT NULL) OR (varINFAR006aBatchId IS NOT NULL) THEN
        
           --Open programUnit_Cursor
           OPEN programUnit_Cursor;
           LOOP
               FETCH programUnit_Cursor INTO varProgramUnit;
               EXIT WHEN programUnit_Cursor%NOTFOUND;
               
               DBMS_OUTPUT.PUT_LINE('Program Unit ' || varProgramUnit);
           
                IF (varINFAR001BatchId IS NOT NULL) THEN   

                   --Clear INFAR001 Counter Data
                   GET_INFAR001_COUNTER_DATA( varINFAR001BatchId, varProgramUnit, NULL, 0, c_EMPTY_COUNTER, V_SUCCESS_FLAG, V_MESSAGE);
           
                END IF;

               --DBMS_OUTPUT.PUT_LINE('Clear INFAR001 Count Data');

                IF (varINFAR006sBatchId IS NOT NULL) THEN   

                   --Clear INFAR006 Setup Counter Data
                   GET_INFAR006_COUNTER_DATA( varINFAR006sBatchId, varProgramUnit, NULL, 0, c_EMPTY_COUNTER, V_SUCCESS_FLAG, V_MESSAGE);
           
                END IF;
               --DBMS_OUTPUT.PUT_LINE('Clear INFAR006 Setup Count Data');

                IF (varINFAR006aBatchId IS NOT NULL) THEN   
                   
                   --Clear INFAR006 Adjustment Counter Data
                   GET_INFAR006_COUNTER_DATA( varINFAR006aBatchId, varProgramUnit, NULL, 0, c_EMPTY_COUNTER, V_SUCCESS_FLAG, V_MESSAGE);

                END IF;           
               --DBMS_OUTPUT.PUT_LINE('Clear INFAR006 Adjustment Count Data');

                IF (varINFAR001BatchId IS NOT NULL) THEN              

                   --Process INFAR001 Data
                   FS_PROCESS_INFAR001(varINFAR001BatchId, varBatchDateStr, varProgramUnit);

                END IF;
         
                IF (varINFAR006sBatchId IS NOT NULL) THEN   

                   --Process INFAR006 Setup Data
                   FS_PROCESS_INFAR006_SETUP(varINFAR006sBatchId, varBatchDateStr, varProgramUnit, varLastBatchDate);

                END IF;
               
               --DBMS_OUTPUT.PUT_LINE('Gather INFAR006 Setup Data');

                IF (varINFAR006aBatchId IS NOT NULL) THEN   
                   
                   --Process INFAR006 Adjustment Data
                   FS_PROCESS_INFAR006_ADJUST(varINFAR006aBatchId, varBatchDateStr, varProgramUnit, varLastBatchDate);

                END IF;
               --DBMS_OUTPUT.PUT_LINE('Gather INFAR006 Adjustment Data');

                IF (varINFAR001BatchId IS NOT NULL) THEN  
                
                   -- Update INFAR001 Counter Data.
                   UPD_INFAR001_COUNTER_DATA(varINFAR001BatchId, varProgramUnit, NULL, NULL, V_SUCCESS_FLAG, V_MESSAGE);

                END IF;

                IF (varINFAR006sBatchId IS NOT NULL) THEN          

                   -- Update INFAR006 Setup Counter Data.
                   UPD_INFAR006_COUNTER_DATA(varINFAR006sBatchId, varProgramUnit, NULL, NULL, V_SUCCESS_FLAG, V_MESSAGE);
                   
                   DBMS_OUTPUT.PUT_LINE('Update INFAR006 Setup Counter Data '||V_MESSAGE);

                END IF;

                IF (varINFAR006aBatchId IS NOT NULL) THEN   

                   -- Update INFAR006 Adjustment Counter Data.
                   UPD_INFAR006_COUNTER_DATA(varINFAR006aBatchId, varProgramUnit, NULL, NULL, V_SUCCESS_FLAG, V_MESSAGE);
                   
                   DBMS_OUTPUT.PUT_LINE('Update INFAR006 Adjustment Counter Data'||V_MESSAGE);
                   
                END IF;
           
           END LOOP; -- for all program units
           
       END IF; -- If any one batch is created
       
        IF (varINFAR001BatchId IS NOT NULL) THEN  

           --Update INFAR001 Batch Count and Total Amount in Batch table
           INFAR001_UPDATE_BATCH(varINFAR001BatchId, varBatchDateStr);
        ...
        IF (varINFAR006sBatchId IS NOT NULL) THEN   
           ...
           --Update INFAR006 Batch Count and Total Amount in BATCH table
           INFAR006_UPDATE_BATCH(varINFAR006sBatchId);
        ...
        IF (varINFAR006aBatchId IS NOT NULL) THEN   
           ...
           --Update INFAR006 Batch Count and Total Amount in BATCH table
           INFAR006_UPDATE_BATCH(varINFAR006aBatchId);
        ...
        IF (varINFAR001BatchId IS NOT NULL) OR (varINFAR006sBatchId IS NOT NULL) OR (varINFAR006aBatchId IS NOT NULL) THEN
            ...
            V_SUCCESS_FLAG := C_YES;
            V_MESSAGE := 'FISCAL Batch Data  for ' || varBatchDateStr ||' ' || 'Populated.';

            COMMIT;
            
            -- 5/1/2018, Vinay Patil: Log the Successful processing message to CARS ERROR LOG table in CARS schema.
            LOG_CARS_ERROR(
                p_errorLevel    => V_LEVEL,
        ...
        ELSIF (varINFAR001BatchId IS NULL) AND (varINFAR006sBatchId IS NULL) AND (varINFAR006aBatchId IS NULL) THEN

            V_SUCCESS_FLAG := C_NO;
            V_MESSAGE := 'FS_BATCH_DRIVER: FISCAL Batch Data  for ' || varBatchDateStr ||' ' || ' not created.';
                   
            -- 5/1/2018, Vinay Patil: Log the message to CARS ERROR LOG table in CARS schema.
            V_LEVEL         := '3';
            V_SEVERITY      := c_HIGH_SEVERITY;    
            V_ERROR_CODE    := '5000';
            V_ERROR_DETAIL  := 'FS_BATCH_DRIVER: FISCAL AR Setup, Adjustment and Payment batches on '||sysdate||' were not created.';    
    ...
    EXCEPTION
          WHEN NO_DATA_FOUND THEN
                ...
                -- Consider logging the error and then re-raise          
                V_SUCCESS_FLAG  := c_NO;
                V_MESSAGE       := 'FS_BATCH_DRIVER : ' || SUBSTR(SQLERRM, 1 , 100);
                ...
                V_SEVERITY      := c_HIGH_SEVERITY;    
                V_ERROR_CODE    := '5000';
                V_ERROR_DETAIL  := 'FISCAL AR Setup, Adjustment and Payment data processing on '||sysdate||' failed as no data was found.';    
          ...
          WHEN OTHERS THEN
                ...
                V_SEVERITY      := c_HIGH_SEVERITY;    
                V_ERROR_CODE    := '5000';
                V_ERROR_DETAIL  := 'FISCAL AR Setup, Adjustment and Payment data processing on '||sysdate||' failed.';    
    ...
    PROCEDURE GET_INFAR006_COUNTER_DATA(
       ...
       V_BATCH_ID                           INFAR006_OUTBOUND.BATCH_ID%TYPE;
       V_INFAR006_OUTBOUND_ID               INFAR006_OUTBOUND.INFAR006_OUTBOUND_ID%TYPE;
       ...
       V_DATA_SOURCE                        INFAR006_OUTBOUND.DATA_SOURCE_CODE%TYPE;
       V_PROGRAM_UNIT                       PROGRAM_UNIT.PROGRAM_UNIT_CODE %TYPE;

       V_RECORD_ID                          INFAR006_OUTBOUND.BATCH_ID%TYPE;
       V_GROUP_ID_PREFIX                    INFAR006_OUTBOUND.GROUP_ID_STG%TYPE;
       V_GROUP_ID_SEQUENCE                  NUMBER                  := 0;    
       V_AR_LINES_IN_GROUP                  NUMBER                  := 1; --99;-- FISCAL allows 99 lines per group,however Accounting wants 1 line per group
    ...
    BEGIN
        ...
        /*  
        REFENCE CODE LIST DOMAIN = GROUP_ID_PREFIX
        EVINF      EV        
        PVINF      PV 
        OSHAINF    CALOSHA
        ARTINF     ART
        ...
        */
        ...
        IF ( (P_BATCH_ID IS NOT NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) AND (P_ACTION IS NOT NULL) ) THEN


            IF (P_ACTION = c_EMPTY_COUNTER) THEN
            
                V_INFAR006_COUNT_REC := V_INFAR006_EMPTY_REC;
            
                DBMS_OUTPUT.PUT_LINE ('The Counter Array used for INFAR006 is emptied. Batch ID is ' ||   P_BATCH_ID  );

                v_INFAR006_TBL.DELETE;   
                               
            END IF;
            
            IF (P_ACTION = c_DELETE_LAST) THEN
            
                IF (v_INFAR006_TBL.COUNT > 0) THEN
           
                    DBMS_OUTPUT.PUT_LINE ('Deleted the row '||v_INFAR006_TBL.COUNT||' of the INFAR006 Counter Array ' ||    V_INFAR006_COUNT_REC.BATCH_ID   );
            ...
            --4/02/2018   Vinaykumar Patil   1 Added lookup values based on FISCAL Batch Processing, Control the Group items
            BEGIN
                SELECT TO_NUMBER(RCL.REFERENCE_CODE) AR_LINES
                INTO   V_AR_LINES_IN_GROUP
                FROM   REFERENCE_CODE_LIST RCL
                WHERE  RCL.DOMAIN_NAME          = 'ARLINES_IN_GROUP'
                AND    RCL.REFERENCE_VALUE      = 'AR_LINES'
                AND    RCL.EFFECTIVE_END_DATE IS NULL;
            
            EXCEPTION
                WHEN OTHERS THEN
                    V_AR_LINES_IN_GROUP := 1;
            
            END;

            IF (P_ACTION = c_ADD_COUNTER) THEN 

                IF (V_GROUP_ID_PREFIX IS NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) THEN 

                    SELECT  RCL.REFERENCE_CODE
                    INTO    V_GROUP_ID_PREFIX
                    FROM    REFERENCE_CODE_LIST RCL
                    WHERE   RCL.DOMAIN_NAME = c_GROUP_PREFIX
                    AND     RCL.REFERENCE_VALUE = P_PROGRAM_UNIT_CODE;

                END IF;

                V_RECORD_ID                                := NVL(V_INFAR006_COUNT_REC.RECORD_ID, 0) + 1;
                V_INFAR006_COUNT_REC.RECORD_ID             := V_RECORD_ID;
                V_INFAR006_COUNT_REC.BATCH_ID              := P_BATCH_ID;
                V_INFAR006_COUNT_REC.PROGRAM_UNIT_CODE     := P_PROGRAM_UNIT_CODE;
                V_INFAR006_COUNT_REC.DATA_SOURCE_CODE      := P_DATA_SOURCE_CODE;
               
                -- Add the Program Unit Group AR Line Count and get the next group id
                IF ( NVL(V_INFAR006_COUNT_REC.GROUP_SEQ_NUM,0) BETWEEN 1 AND V_AR_LINES_IN_GROUP ) THEN
                         
                    V_INFAR006_COUNT_REC.GROUP_SEQ_NUM      := V_INFAR006_COUNT_REC.GROUP_SEQ_NUM + 1;
                
                    V_INFAR006_COUNT_REC.GROUP_ID_STG       := V_INFAR006_COUNT_REC.GROUP_ID_STG;

                    -- Add AR Setup or Reversal Amounts
                    V_INFAR006_COUNT_REC.CONTROL_AMT        := NVL(V_INFAR006_COUNT_REC.CONTROL_AMT,0.000) + NVL(P_CONTROL_AMT,0.00);         
                           
                    -- Add the Total AR Line Count
                    V_INFAR006_COUNT_REC.CONTROL_CNT        := NVL(V_INFAR006_COUNT_REC.CONTROL_CNT,0) +  1;
                    
                ELSE
                
                    -- Group Sequence Number
                    V_INFAR006_COUNT_REC.GROUP_SEQ_NUM      := 1;
                    
                    -- Add AR Setup or Reversal Amounts
                    V_INFAR006_COUNT_REC.CONTROL_AMT        := NVL(P_CONTROL_AMT,0.00);         
                           
                    -- Add the Total AR Line Count
                    V_INFAR006_COUNT_REC.CONTROL_CNT        := 1;

                    -- Per Liserin Lau of Accounting, We need to have Group id unique in FISCAL system therefore we need to use sequence object
                    -- Generate GROUP ID STG Counter
                    SELECT FISCAL_GROUP_ID_STG_SEQ.NEXTVAL INTO V_GROUP_ID_SEQUENCE FROM DUAL;

                   -- IF (V_INFAR006_COUNT_REC.GROUP_ID_STG IS NULL) THEN 
                   --    V_GROUP_ID_SEQUENCE         :=  1; -- Counter
                   -- ELSE
                   --     V_GROUP_ID_SEQUENCE         := LTRIM(V_INFAR006_COUNT_REC.GROUP_ID_STG,V_GROUP_ID_PREFIX) + 1; -- Counter                    
                   -- END IF;

                    V_INFAR006_COUNT_REC.GROUP_ID_STG       := V_GROUP_ID_PREFIX || TO_CHAR(V_GROUP_ID_SEQUENCE);  -- Formatted Counter 

                END IF;

                -- Initialize and Assign like extending the Collection and Assigning.
                v_INFAR006_TBL.extend; 
                v_INFAR006_TBL(V_RECORD_ID) := V_INFAR006_COUNT_REC;
                    
                DBMS_OUTPUT.PUT_LINE (
                    V_INFAR006_COUNT_REC.RECORD_ID                || ', ' ||
                    V_INFAR006_COUNT_REC.BATCH_ID                 || ', ' ||
                    V_INFAR006_COUNT_REC.PROGRAM_UNIT_CODE        || ', ' ||
                    V_INFAR006_COUNT_REC.DATA_SOURCE_CODE         || ', ' ||
                    V_GROUP_ID_SEQUENCE                           || ', ' ||   
                    V_INFAR006_COUNT_REC.GROUP_ID_STG             || ', ' ||
                    V_INFAR006_COUNT_REC.GROUP_SEQ_NUM            || ', ' ||
                    V_INFAR006_COUNT_REC.CONTROL_AMT              || ', ' ||
                    V_INFAR006_COUNT_REC.CONTROL_CNT  
                 );
        ...
        ELSE
                ...
                V_MESSAGE          := 'One of the Parameter value to the procedure is not provided' 
                                    ...
                                    ||' P_PROGRAM_UNIT_CODE = '||P_PROGRAM_UNIT_CODE
                                    ||' P_ACTION = '||P_ACTION;            

                DBMS_OUTPUT.PUT_LINE(
                                        ' V_SUCCESS_FLAG = '||V_SUCCESS_FLAG||' '||V_MESSAGE   );          
        END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
                V_SUCCESS_FLAG     := c_NO;
                V_MESSAGE          := 'Failure occured in GET_INFAR006_COUNTER_DATA :' ||SQLERRM;
				...
				LOG_CARS_ERROR(
					p_errorLevel    => '3',
					p_severity      => c_HIGH_SEVERITY,
					p_errorDetail   => 'FISCAL GET_INFAR006_COUNTER_DATA procedure did not succeed',
					p_errorCode     => '5000',
					p_errorMessage  => null ,
					p_dataSource    => c_CARS_DB
				    ); 

    END GET_INFAR006_COUNTER_DATA; 
    ...
    BEGIN
        ...
        IF (P_BATCH_ID IS NOT NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) THEN
            
            FOR v_REC IN 
                    (SELECT  A.*
                     FROM    TABLE (FS_INTERFACE_PKG.V_INFAR006_TBL) A  
                     WHERE   A.RECORD_ID IN (SELECT MAX(B.RECORD_ID) FROM TABLE (FS_INTERFACE_PKG.V_INFAR006_TBL) B GROUP BY B.GROUP_ID_STG)
                    ) 
            LOOP
                    DBMS_OUTPUT.PUT_LINE(' '
                                        ||' Record Id = '           ||v_REC.RECORD_ID
                                        ||' Batch Id = '            ||v_REC.BATCH_ID 
                                        ||' Control Amount = '      ||v_REC.CONTROL_AMT
                                        ||' Control Count = '       ||v_REC.CONTROL_CNT
                                        ||' Group Id Stg = '        ||v_REC.GROUP_ID_STG
                                        ||' Group Sequence Number = '||v_REC.GROUP_SEQ_NUM
                                        );
                                            
                    UPDATE  INFAR006_OUTBOUND
                    SET     CONTROL_AMT     = v_REC.CONTROL_AMT, 
                            CONTROL_CNT     = v_REC.CONTROL_CNT
                    WHERE   BATCH_ID        = v_REC.BATCH_ID 
                    AND     GROUP_ID_STG    = v_REC.GROUP_ID_STG    
                    ;
    ...
    EXCEPTION
        WHEN OTHERS THEN 
                ...
                V_SUCCESS_FLAG     := c_NO;
                V_MESSAGE          := 'Failure occured in UPD_INFAR006_COUNTER_DATA :' ||SQLERRM;
    ...
    PROCEDURE GET_INFAR001_COUNTER_DATA(
       ...
       V_BATCH_ID                   INFAR001_OUTBOUND.BATCH_ID%TYPE;
       V_INFAR001_OUTBOUND_ID       INFAR001_OUTBOUND.INFAR001_OUTBOUND_ID%TYPE;
       V_CONTROL_AMT                INFAR001_OUTBOUND.CONTROL_AMT%TYPE;
       V_CONTROL_CNT                INFAR001_OUTBOUND.CONTROL_CNT%TYPE;
       V_DATA_SOURCE                INFAR001_OUTBOUND.DATA_SOURCE_CODE%TYPE;
       V_PROGRAM_UNIT               PROGRAM_UNIT.PROGRAM_UNIT_CODE %TYPE;
       V_DEPOSIT_ID_PREFIX          INFAR001_OUTBOUND.DEPOSIT_ID%TYPE;
       V_RECORD_ID                  INFAR001_OUTBOUND.BATCH_ID%TYPE;       

       V_DEPOSIT_ID_SEQUENCE        NUMBER         := 0;       
       V_SUCCESS_FLAG               VARCHAR2(1)    := c_YES;
       V_MESSAGE                    VARCHAR2(500);

    BEGIN

--        REFENCE CODE LIST DOMAIN = DEPOSIT_ID_PREFIX
--        CASH      CSH
--        PSM       PSM
--        DWC       DWC
--        OSIP      OSIP
...
--        OSHA      CALOSHA
        ...
        IF ( (P_BATCH_ID IS NOT NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) AND (P_ACTION IS NOT NULL) ) THEN
            ...
            IF (P_ACTION = c_EMPTY_COUNTER) THEN
                ...
                V_INFAR001_COUNT_REC := V_INFAR001_EMPTY_REC;
            
                DBMS_OUTPUT.PUT_LINE ('The Counter Array used for INFAR001 is emptied. Batch ID is ' ||   P_BATCH_ID  );
               
                v_INFAR001_TBL.DELETE; 
                
            END IF;

            IF (P_ACTION = c_DELETE_LAST) THEN
            
                IF (v_INFAR001_TBL.COUNT > 0) THEN
           
                    DBMS_OUTPUT.PUT_LINE ('Deleted the row '||v_INFAR001_TBL.COUNT||' of the INFAR006 Counter Array ' ||    V_INFAR001_COUNT_REC.BATCH_ID   );
            ...
            IF (P_ACTION IN (c_DC_ROW_ID,c_PI_ROW_ID,c_IR_ROW_ID,c_CI_ROW_ID)) THEN 

                IF (V_DEPOSIT_ID_PREFIX IS NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) THEN 

                    SELECT  RCL.REFERENCE_CODE
                    INTO    V_DEPOSIT_ID_PREFIX
                    FROM    REFERENCE_CODE_LIST RCL
                    WHERE   RCL.DOMAIN_NAME     = c_DEPOSIT_PREFIX
                    AND     RCL.REFERENCE_VALUE = P_PROGRAM_UNIT_CODE;

                END IF;

                V_RECORD_ID                                := NVL(V_INFAR001_COUNT_REC.RECORD_ID, 0) + 1;
                V_INFAR001_COUNT_REC.RECORD_ID             := V_RECORD_ID;
                V_INFAR001_COUNT_REC.BATCH_ID              := P_BATCH_ID;
                V_INFAR001_COUNT_REC.PROGRAM_UNIT_CODE     := P_PROGRAM_UNIT_CODE;
                V_INFAR001_COUNT_REC.DATA_SOURCE_CODE      := P_DATA_SOURCE_CODE;

                -- Generate the Deposit Id
                IF (P_ACTION IN (c_DC_ROW_ID)) THEN

                    -- Set or Reset  Payment Sequence Number
                    V_INFAR001_COUNT_REC.PAYMENT_SEQ_NUM    := 0;

                    -- Set or Reset  AR Item Reference count
                    V_INFAR001_COUNT_REC.ID_SEQ_NUM         := 0;

                   -- Set or Reset Distribution Journal Line count
                    V_INFAR001_COUNT_REC.DST_SEQ_NUM        := 0;
                    
                    -- Set or Reset Payment Amount Calculation
                    V_INFAR001_COUNT_REC.CONTROL_AMT        := 0.00;         
                           
                    -- Set or Reset Payment Counter
                    V_INFAR001_COUNT_REC.CONTROL_CNT        := 0;
                    
                    -- Set or Reset Total Deposit Amount Calculation
                    V_INFAR001_COUNT_REC.TOTAL_AMT          := NVL(V_INFAR001_COUNT_REC.TOTAL_AMT,0) + 0.00; 

                    -- Set or Reset Deposit Counter
                    V_INFAR001_COUNT_REC.DEPOSIT_CNT        := NVL(V_INFAR001_COUNT_REC.DEPOSIT_CNT,0) + 1;
                    
                    -- Per Liserin Lau of Accounting, We need to have Deposit id unique in FISCAL system therefore we need to use sequence object
                    -- Generate DEPOSIT ID Counter
                    SELECT FISCAL_DEPOSIT_ID_SEQ.NEXTVAL INTO V_DEPOSIT_ID_SEQUENCE FROM DUAL;
                    
                   -- IF (V_INFAR001_COUNT_REC.DEPOSIT_ID IS NULL) THEN 
                   --     V_DEPOSIT_ID_SEQUENCE               :=  1; -- Counter
                   -- ELSE
                   --     V_DEPOSIT_ID_SEQUENCE               := LTRIM(V_INFAR001_COUNT_REC.DEPOSIT_ID,V_DEPOSIT_ID_PREFIX) + 1; -- Counter                    
                   -- END IF;
                    
                    V_INFAR001_COUNT_REC.DEPOSIT_ID         := V_DEPOSIT_ID_PREFIX || TO_CHAR(V_DEPOSIT_ID_SEQUENCE);  -- Formatted Counter 

                END IF;
                                
                -- Add the Deposit Id line
                IF (P_ACTION IN (c_PI_ROW_ID)) THEN
            
                    -- Maintain the Deposit Id
                    V_INFAR001_COUNT_REC.DEPOSIT_ID         := V_INFAR001_COUNT_REC.DEPOSIT_ID; 
                    
                    -- Payment Sequence Number
                    V_INFAR001_COUNT_REC.PAYMENT_SEQ_NUM    := NVL(V_INFAR001_COUNT_REC.PAYMENT_SEQ_NUM,0) +  1;
                    
                    -- Add Payment or Reversal Amounts
                    V_INFAR001_COUNT_REC.CONTROL_AMT        := V_INFAR001_COUNT_REC.CONTROL_AMT + NVL(P_CONTROL_AMT,0.00); 
                           
                    -- Add the Total Payment Line Count
                    V_INFAR001_COUNT_REC.CONTROL_CNT        := V_INFAR001_COUNT_REC.CONTROL_CNT +  1;

                    -- Add Payment or Reversal Amount to Total Deposit Amount Calculation
                    V_INFAR001_COUNT_REC.TOTAL_AMT          := V_INFAR001_COUNT_REC.TOTAL_AMT + NVL(P_CONTROL_AMT,0.00); 

                    -- Add Deposit Line to Total Deposit Counter
                    V_INFAR001_COUNT_REC.DEPOSIT_CNT        := V_INFAR001_COUNT_REC.DEPOSIT_CNT +  0;

                END IF;
               
                -- Add the Program Unit Group AR Line Count and get the next group id
                IF (P_ACTION IN (c_IR_ROW_ID)) THEN
       
                    -- Maintain the Deposit Id
                    V_INFAR001_COUNT_REC.DEPOSIT_ID         := V_INFAR001_COUNT_REC.DEPOSIT_ID;

                    -- Maintain the Payment Sequence Number
                    V_INFAR001_COUNT_REC.PAYMENT_SEQ_NUM    := V_INFAR001_COUNT_REC.PAYMENT_SEQ_NUM;

                    -- Add AR Item Reference count
                    V_INFAR001_COUNT_REC.ID_SEQ_NUM         := 1; --NVL(V_INFAR001_COUNT_REC.ID_SEQ_NUM,0) +  1;
                                    
                    -- Maintain AR Setup or Reversal Amounts
                    V_INFAR001_COUNT_REC.CONTROL_AMT        := V_INFAR001_COUNT_REC.CONTROL_AMT + 0.00;         
                           
                    -- Maintain the Total AR Line Count
                    V_INFAR001_COUNT_REC.CONTROL_CNT        := V_INFAR001_COUNT_REC.CONTROL_CNT +  0;
                    
                    -- Maintain the Total Deposit Amount Calculation
                    V_INFAR001_COUNT_REC.TOTAL_AMT          := V_INFAR001_COUNT_REC.TOTAL_AMT + 0.00; 

                    -- Maintain the Total Deposit Counter
                    V_INFAR001_COUNT_REC.DEPOSIT_CNT        := V_INFAR001_COUNT_REC.DEPOSIT_CNT +  0;

                END IF;

                -- Add the Program Unit Group AR Line Count and get the next group id
                IF (P_ACTION IN (c_CI_ROW_ID)) THEN
                    ...
                    -- Maintain the AR item reference count
                    V_INFAR001_COUNT_REC.ID_SEQ_NUM         := V_INFAR001_COUNT_REC.ID_SEQ_NUM;
                    
                    -- Add Distribution Journal Line count
                    V_INFAR001_COUNT_REC.DST_SEQ_NUM         := NVL(V_INFAR001_COUNT_REC.DST_SEQ_NUM,0) +  1;
                ...
                -- Initialize and Assign like extending the Collection and Assigning.
                v_INFAR001_TBL.extend; 
                v_INFAR001_TBL(V_RECORD_ID) := V_INFAR001_COUNT_REC;
                    
                DBMS_OUTPUT.PUT_LINE (
                    V_INFAR001_COUNT_REC.RECORD_ID                || ', ' ||
                    V_INFAR001_COUNT_REC.BATCH_ID                 || ', ' ||
                    V_INFAR001_COUNT_REC.PROGRAM_UNIT_CODE        || ', ' ||
                    V_INFAR001_COUNT_REC.DATA_SOURCE_CODE         || ', ' ||
                    V_INFAR001_COUNT_REC.DEPOSIT_ID               || ', ' ||  
                    V_DEPOSIT_ID_SEQUENCE                         || ', ' ||
                    V_INFAR001_COUNT_REC.DEPOSIT_CNT              || ', ' ||
                    V_INFAR001_COUNT_REC.PAYMENT_SEQ_NUM          || ', ' ||
                    V_INFAR001_COUNT_REC.ID_SEQ_NUM               || ', ' ||
                    V_INFAR001_COUNT_REC.DST_SEQ_NUM              || ', ' ||
                    V_INFAR001_COUNT_REC.TOTAL_AMT                || ', ' ||
                    V_INFAR001_COUNT_REC.CONTROL_AMT              || ', ' ||
                    V_INFAR001_COUNT_REC.CONTROL_CNT  
                 );
            END IF;          
    ...
    EXCEPTION
        WHEN OTHERS THEN
                V_SUCCESS_FLAG     := c_NO;
                V_MESSAGE          := 'Failure occured in GET_INFAR001_COUNTER_DATA :' ||SQLERRM;
                
                P_SUCCESS_FLAG    := V_SUCCESS_FLAG;
                P_MESSAGE         := V_MESSAGE;
                DBMS_OUTPUT.PUT_LINE(' V_SUCCESS_FLAG = '||V_SUCCESS_FLAG||' ' ||V_MESSAGE );
                
    END GET_INFAR001_COUNTER_DATA; 
        ...
        IF (P_BATCH_ID IS NOT NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) THEN
            ...
            FOR v_REC IN 
                    (SELECT  A.*
                     ...
                     WHERE   A.RECORD_ID IN (SELECT MAX(B.RECORD_ID) FROM TABLE (FS_INTERFACE_PKG.V_INFAR001_TBL) B GROUP BY B.DEPOSIT_ID)
                    ) 
            LOOP
                ...
                DBMS_OUTPUT.PUT_LINE(' '
                                        ...
                                        ||' Control Amount = '      ||v_REC.CONTROL_AMT
                                        ||' Control Count = '       ||v_REC.CONTROL_CNT
                                        ||' Deposit Id  = '         ||v_REC.DEPOSIT_ID
                                        ||' Payment Sequence Number = '||v_REC.PAYMENT_SEQ_NUM
                                        );
    
                    UPDATE  INFAR001_OUTBOUND IO
                    SET     IO.CONTROL_AMT     = v_REC.CONTROL_AMT, 
                            IO.CONTROL_CNT     = v_REC.CONTROL_CNT
                    WHERE   IO.BATCH_ID        = v_REC.BATCH_ID 
                    AND     IO.DEPOSIT_ID      = v_REC.DEPOSIT_ID  
                    AND     IO.FS_ROW_ID       = c_DC_ROW_ID  
                ;
            END LOOP;
    ...
    EXCEPTION
        WHEN OTHERS THEN 
                ...
                V_SUCCESS_FLAG     := c_NO;
                V_MESSAGE          := 'Failure occured in UPD_INFAR001_COUNTER_DATA :' ||SQLERRM;
               ...
               LOG_CARS_ERROR(
					p_errorLevel    => '3',
					p_severity      => c_HIGH_SEVERITY,
					p_errorDetail   => 'FISCAL UPD_INFAR001_COUNTER_DATA procedure did not succeed',
					p_errorCode     => '5000',
					p_errorMessage  => V_MESSAGE ,
					p_dataSource    => c_CARS_DB
				    );  
    END UPD_INFAR001_COUNTER_DATA;    
    ...
    BEGIN
        ...
        IF (P_BATCH_ID IS NOT NULL) AND (P_PROGRAM_UNIT_CODE IS NOT NULL) THEN
            ...
            FOR v_REC IN 
                    (SELECT  A.*
                     FROM    TABLE (FS_INTERFACE_PKG.V_INFAR001_TBL) A  
                     WHERE   A.RECORD_ID IN (SELECT MAX(B.RECORD_ID) FROM TABLE (FS_INTERFACE_PKG.V_INFAR001_TBL) B )
                    ) 
            LOOP
    ...
    EXCEPTION
        WHEN OTHERS THEN 
                ...
                V_SUCCESS_FLAG     := c_NO;
                V_MESSAGE          := 'Failure occured in UPD_INFAR001_HEADER_DATA :' ||SQLERRM;
				...
				LOG_CARS_ERROR(
					p_errorLevel    => '3',
					p_severity      => c_HIGH_SEVERITY,
					p_errorDetail   => 'FISCAL UPD_INFAR001_HEADER_DATA procedure did not succeed',
					p_errorCode     => '5000',
					p_errorMessage  => V_MESSAGE ,
					p_dataSource    => c_CARS_DB
				    );
